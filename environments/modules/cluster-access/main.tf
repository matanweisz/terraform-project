...
resource "aws_security_group_rule" "allow_internal_cluster_to_control_plane" {
  description              = "Allow internal cluster to communicate with ${var.environment} cluster API server"
  type                     = "ingress"
  from_port                = 443
  to_port                  = 443
  protocol                 = "tcp"
